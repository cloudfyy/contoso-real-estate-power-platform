...
if ([string]::IsNullOrWhiteSpace($ENTRA_API_OBJECT_ID)) {
	Write-Host "Skipping API application deletion because ENTRA_API_OBJECT_ID is empty."
}
else {
...
if ([string]::IsNullOrWhiteSpace($ENTRA_API_CLIENT_OBJECT_ID)) {
	Write-Host "Skipping API client application deletion because ENTRA_API_CLIENT_OBJECT_ID is empty."
}
else {
