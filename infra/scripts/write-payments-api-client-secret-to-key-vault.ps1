...
function Get-PaymentsApiAccessTokenWithRole {
    ...
    throw "Could not acquire an access token containing audience '$expectedAudience' and role '$RequiredRole'. The app role assignment or admin consent might still be propagating for client app '$ApiClientAppId'. Rerun this script after a few minutes if the role was just assigned."
}
